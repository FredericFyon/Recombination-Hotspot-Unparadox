...
vector<vector<vector<double> > >Meiosis(vector<vector<double> > Indiv, double R, double mu_A, double mu_B, double b, double c, double f, int Nloc, mt19937 mt)
{
    ...
    if(attempt < 1./4.)                                                       //With probability 1/4 a given PRDM9 allele attempts to bind a given target allele
                                                                            //at the drawn target locus
    {
        if((IndivBuffer[0][1] == IndivBuffer[tirageLocTarget+1][0]) && success <= b)     //If the PRDM9 allele 1 and target allele 1 match, and with probability b
    ...
    else if(attempt >= 1./4. && attempt < 1./2.)
    {
        if((IndivBuffer[0][1] == IndivBuffer[tirageLocTarget+1][1]) && success <= b)     //PRDM9 allele 1 and target allele 2
        {
            vector<double> WW;
            WW.push_back(1.);
            W.push_back(WW);
    ...
    else if(attempt >= 1./2. && attempt < 3./4.)
    {
        if((IndivBuffer[0][3] == IndivBuffer[tirageLocTarget+1][0]) && success <= b)     //PRDM9 allele 2 and target allele 1
        {
            vector<double> WW;
            WW.push_back(1.);
            W.push_back(WW);
    ...
    {
        if((IndivBuffer[0][3] == IndivBuffer[tirageLocTarget+1][1]) && success <= b)     //PRDM9 allele 2 and target allele 2
        {
            vector<double> WW;
            WW.push_back(1.);
            W.push_back(WW);
